...
- **Frontend**: Svelte 5 + SvelteKit (adapter-static) + Tailwind CSS
- **Backend**: FastAPI + Uvicorn (Python, wraps existing shell scripts)
- **Web Server**: Nginx (replaces Lighttpd for WebSocket proxy support)
- **Database**: SQLite (replaces 50+ flat config files)
- **Real-time**: WebSocket with MPD idle bridge
